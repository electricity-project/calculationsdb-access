...
public interface PowerProductionRepository extends JpaRepository<PowerProduction, Long> {
    ...
    List<PowerProduction> getByIpv6OrderByTimestampDesc(String ipv6, ZonedDateTime filterDate);
    ...
    List<IPowerAggregationProduction> getByIpv6AndTimestampForHour(String ipv6, ZonedDateTime filterDate);
    ...
    @Query(value = "select date_trunc('day', timestamp) as aggregatedTimestamp, SUM(produced_power) as aggregatedValue" +
            ...
            " order by aggregatedTimestamp DESC", nativeQuery = true)
    List<IPowerAggregationProduction> getByIpv6AndTimestampForDay(String ipv6, ZonedDateTime filterDate);

    List<PowerProduction> getByTimestampGreaterThanEqualAndTimestampLessThan(ZonedDateTime startingDate, ZonedDateTime endingDate);
}